...
namespace dbm
{
    ...
    public:
         ...
         */
        partition_t(cindex_t dim, bool noPartition = false): fedTable(dimFlag2ptr(dim, noPartition)) {}

        /// The copy is a reference copy only.
        partition_t(const partition_t& arg): fedTable(arg.fedTable)
        {
            assert(fedTable);
            if (isPtr())
                fedTable->incRef();
        ...
        bool isPartition() const { return !(isPtr() ? fedTable->isNotPartition() : eflag()); }
        ...
        {
        public:
            entry_t(uintptr_t i, const fed_t& f): id(i), fed(f) {}

            uintptr_t id;  //< ID of the federation, subset of the partition.
        ...
        public:
            ...
            const_iterator(const entry_t* const* start): current(start) {}
            ...
            /// nothing left.
            bool operator==(const const_iterator& arg) const { return current == arg.current; }
            bool operator!=(const const_iterator& arg) const { return !(*this == arg); }
            bool operator<(const const_iterator& arg) const { return current < arg.current; }
        ...
        {
            assert(fedTable);
            return isPtr() ? const_iterator(fedTable->getBeginTable()) : const_iterator(NULL);
        }
        const_iterator end() const
        {
            assert(fedTable);
            return isPtr() ? const_iterator(fedTable->getEndTable()) : const_iterator(NULL);
        }
    ...
    private:
        ...
        public:
            ...
            bool add(uintptr_t id, fed_t& fed);
        ...
        private:
            ...
            fedtable_t(cindex_t d, bool noP): refCounter(1), all(d), nbEntries(0), mask(INIT_MASK), noPartition(noP)
            {
        ...
        // this assumes that the alignment is sound (i.e. lsb is unused).
        static_assert((alignof(fedtable_t) % 2) != 1);
        static fedtable_t* dimFlag2ptr(cindex_t dim, bool flag)
        {
            return reinterpret_cast<fedtable_t*>((dim << 2) | (flag << 1) | 1);
        }
        bool isPtr() const { return (((uintptr_t)fedTable) & 1) == 0; }
        cindex_t edim() const
        {
            assert(!isPtr());
