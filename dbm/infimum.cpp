...
{
    std::cerr << "Index: " << i << ", pred: " << pred(i) << ", depth: " << depth(i)
              << ", thread: " << thread(i) << ",  inbound: " << inbound(i) << ", flow: " << flow(i)
...
static bool checkTreeIntegrity(const raw_t* dbm, const int32_t* rates, Node* nodes, uint32_t dim)
{
    ...
    for (i = 0; i < dim; i++) {
        if (sum[i] != 0) {
            ...
            std::cerr << "sum[" << i << "] is corrupted: " << sum[i]
                      << ", should be: " << (i == 0 ? total : b(i)) << std::endl;
        }
    }
        ...
        if (potential(i) == dbm_INFINITY && pred(i) == 0) {
            ...
            if (reduced_cost != 0) {
                ...
                std::cerr << "Reduced cost not zero (" << reduced_cost << ") i (pot): " << i << " ("
                          << potential(i) << "), pred(i) (pot): " << pred(i) << " ("
                          << potential(pred(i)) << "), inbound: " << inbound(i) << std::endl;
            }
        }
    }
    ...
    for (i = 1; i < dim; i++) {
        if ((depth(pred(i)) + 1) != depth(i)) {
            treeOK = false;
            std::cerr << "Depth of " << i << " is " << depth(i) << ", but depth of pred(i) "
                      << pred(i) << " is " << depth(pred(i)) << std::endl;
        }
    }
...
{
    for (uint32_t i = 0; i < dim; i++) {
        std::cerr << "Val " << i << ": " << valuation[i] << " (rate: " << rates[i] << ")"
                  << std::endl;
    }
}
 ...
 *
 */
static void findInitialSpanningTreeSolution(const raw_t* dbm, uint32_t dim, const int32_t* rates,
                                            Node* nodes)
{
    Node* last = nodes + dim;
    Node* node = nodes;
                                 ...
                                 bool sourceInRootSubtree, uint32_t flow)
{
    ...
    ASSERT(preorderOut->depth <= leave->depth,
           std::cerr << ", pointToLeave: " << pointToLeave << ", lastOut: " << lastOut
                     << ", preorderOut: " << preorderOut << ", depth(preorderOut): "
                     << preorderOut->depth << ". depth(leave): " << leave->depth);
 ...
 * lowest cost.
 */
static arc_t* enteringArcDanzig(arc_t* firstArc, arc_t* lastArc, Node* nodes, const raw_t* dbm,
                                uint32_t dim)
{
    arc_t* best = NULL;
    ...
    while (firstArc != lastArc) {
         ...
         */
        int32_t reduced_cost = constraintValue(firstArc->i, firstArc->j) - potential(firstArc->i) +
                               potential(firstArc->j);
 ...
 */
static void testAndRemoveArtificialArcs(const raw_t* dbm, const int32_t* rates, Node* nodes,
                                        uint32_t dim)
{
    for (uint32_t i = 1; i < dim; i++) {
...
static void infimumNetSimplex(const raw_t* dbm, uint32_t dim, const int32_t* rates, Node* nodes)
{
    ...
    ASSERT(checkTreeIntegrity(dbm, rates, nodes, dim), printAllNodeInfo(nodes, rates, dim));
...
int32_t pdbm_infimum(const raw_t* dbm, uint32_t dim, uint32_t offsetCost, const int32_t* rates)
{
    ...
    for (uint32_t i = 0; i < dim; i++) {
        ASSERT(potential(i) >= 0, std::cerr << "Node: " << i << std::endl;
               printAllNodeInfo(nodes, rates, dim); printClockLowerBounds(dbm, dim));
        /*
...
void pdbm_infimum(const raw_t* dbm, uint32_t dim, uint32_t offsetCost, const int32_t* rates,
                  int32_t* valuation)
{
    if (allPositive(rates, rates + dim)) {
        valuation[0] = 0;
        ...
        for (uint32_t i = 1; i < dim; i++) {
            ASSERT(potential(i) >= 0, std::cerr << "Node: " << i << std::endl;
                   printAllNodeInfo(nodes, rates, dim); printSolution(valuation, rates, dim);
            ...
            if (potential(i) == dbm_INFINITY && pred(i) == 0 && flow(i) > 0) {
                throw std::domain_error(
                    "Infimum is downward unbound, thus, no well-defined infimum valuation.");
            }
