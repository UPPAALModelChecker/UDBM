...
static bool mingraph_isEqualToMinBitMatrix32(const raw_t* dbm, cindex_t dim,
                                             const int32_t* mingraph);
static bool mingraph_isEqualToMinBitMatrix16(const raw_t* dbm, cindex_t dim,
                                             const int32_t* mingraph);
static bool mingraph_isEqualToMinCouplesij32(const raw_t* dbm, cindex_t dim,
                                             const int32_t* mingraph);
static bool mingraph_isEqualToMinCouplesij16(const raw_t* dbm, cindex_t dim,
                                             const int32_t* mingraph);
...
static bool mingraph_isAnalyzedDBMEqualToCopy16(const raw_t* dbm, cindex_t dim, uint32_t* bitMatrix,
                                                size_t* nbConstraints, const int32_t* mingraph);
static bool mingraph_isAnalyzedDBMEqualToMinBitMatrix32(const raw_t* dbm, cindex_t dim,
                                                        uint32_t* bitMatrix, size_t* nbConstraints,
                                                        const int32_t* mingraph);
static bool mingraph_isAnalyzedDBMEqualToMinBitMatrix16(const raw_t* dbm, cindex_t dim,
                                                        uint32_t* bitMatrix, size_t* nbConstraints,
                                                        const int32_t* mingraph);
static bool mingraph_isAnalyzedDBMEqualToMinCouplesij32(const raw_t* dbm, cindex_t dim,
                                                        uint32_t* bitMatrix, size_t* nbConstraints,
                                                        const int32_t* mingraph);
static bool mingraph_isAnalyzedDBMEqualToMinCouplesij16(const raw_t* dbm, cindex_t dim,
                                                        uint32_t* bitMatrix, size_t* nbConstraints,
                                                        const int32_t* mingraph);
static bool mingraph_isAnalyzedDBMEqualError(const raw_t* dbm, cindex_t dim, uint32_t* bitMatrix,
                                             size_t* nbConstraints, const int32_t* mingraph);
...
bool dbm_isAnalyzedDBMEqualToMinDBM(const raw_t* dbm, cindex_t dim, uint32_t* bitMatrix,
                                    size_t* nbConstraints, const int32_t* minDBM)
{
    /* see mingraph_getTypeIndex comments
     */
 ...
 */
bool dbm_isUnpackedEqualToMinDBM(const raw_t* dbm, cindex_t dim, const int32_t* minDBM,
                                 raw_t* unpackBuffer)
{
    ...
    if (mingraph_isMinimal(info)) {
        ...
        return (mingraph_isCoded16(info) ? mingraph_isEqualToCopy16 : mingraph_isEqualToCopy32)(
            dbm, dim, minDBM);
    }
}
 ...
 */
bool mingraph_isAnalyzedDBMEqualToCopy32(const raw_t* dbm, cindex_t dim, uint32_t* bitMatrix,
                                         size_t* nbConstraints, const int32_t* mingraph)
{
                                             ...
                                             const int32_t* mingraph)
{
    ...
    return mingraph_isAnalyzedDBMEqualToMinBitMatrix32(dbm, dim, bitMatrix, &nbConstraints,
                                                       mingraph);
}
                                             ...
                                             const int32_t* mingraph)
{
    uint32_t bitMatrix[bits2intsize(dim * dim)];
    size_t nbConstraints = dbm_analyzeForMinDBM(dbm, dim, bitMatrix);
    return mingraph_isAnalyzedDBMEqualToMinBitMatrix16(dbm, dim, bitMatrix, &nbConstraints,
                                                       mingraph);
}
    ...
    *srcNbConstraints = dbm_cleanBitMatrix(dbm, dim, srcBitMatrix, *srcNbConstraints);
    ...
    if (nbConstraints) /* could be = 0 */
    {
        ...
        for (;;) {
             ...
             */
            difference |=
                (dbm[i * dim + j] ^ *constraints) | (base_getOneBit(srcBitMatrix, i * dim + j) ^ 1);
                                             ...
                                             const int32_t* mingraph)
{
    uint32_t bitMatrix[bits2intsize(dim * dim)];
    size_t nbConstraints = dbm_analyzeForMinDBM(dbm, dim, bitMatrix);
    return mingraph_isAnalyzedDBMEqualToMinCouplesij32(dbm, dim, bitMatrix, &nbConstraints,
                                                       mingraph);
}
                                             ...
                                             const int32_t* mingraph)
{
    uint32_t bitMatrix[bits2intsize(dim * dim)];
    size_t nbConstraints = dbm_analyzeForMinDBM(dbm, dim, bitMatrix);
    return mingraph_isAnalyzedDBMEqualToMinCouplesij16(dbm, dim, bitMatrix, &nbConstraints,
                                                       mingraph);
}
...
static bool mingraph_isEqualError(const raw_t* dbm, cindex_t dim, const int32_t* mingraph)
{
    fprintf(stderr, RED(BOLD) PACKAGE_STRING
            " fatal error: invalid encoded DBM to compare with" NORMAL "\n");
    exit(2);
