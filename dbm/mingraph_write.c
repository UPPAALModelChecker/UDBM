...
static int32_t* mingraph_encode(const raw_t* dbm, cindex_t dim, const uint32_t* bitMatrix,
                                size_t cnt, bool constraints16, allocator_t c_alloc, size_t offset);
...
static int32_t* mingraph_writeMinDBMDim2(const raw_t* dbm, cindex_t dim, allocator_t c_alloc,
                                         size_t offset);
...
static void mingraph_writeMinCouplesij32(int32_t* where, const raw_t* dbm, cindex_t dim,
                                         const uint32_t* bitMatrix, uint32_t cnt, uint32_t bitCode);
static void mingraph_writeMinCouplesij16(int32_t* where, const raw_t* dbm, cindex_t dim,
                                         const uint32_t* bitMatrix, uint32_t cnt, uint32_t bitCode);
static void mingraph_writeMinBitMatrix32(int32_t* where, const raw_t* dbm, cindex_t dim,
                                         const uint32_t* bitMatrix, uint32_t cnt);
static void mingraph_writeMinBitMatrix16(int32_t* where, const raw_t* dbm, cindex_t dim,
                                         const uint32_t* bitMatrix, uint32_t cnt);
 ...
 */
static inline uint32_t* mingraph_jumpInt16(int16_t* ints, size_t n)
{
    ...
    /* dim > 2 ; apply reduction */
        ...
        if (!cnt) {
            ...
            return mingraph_encode(dbm, dim, bitMatrix, cnt,
                                   tryConstraints16 && (dbm_getMaxRange(dbm, dim) < dbm_LS_INF16),
                                   c_alloc, offset);
        }
                              ...
                              size_t offset)
{
    ...
    return mingraph_encode(dbm, dim, bitMatrix, nbConstraints,
                           tryConstraints16 && (dbm_getMaxRange(dbm, dim) < dbm_LS_INF16), c_alloc,
                           offset);
}
                                ...
                                size_t cnt, bool constraints16, allocator_t c_alloc, size_t offset)
{
    ...
    if (sizeIfCopy <= sizeIfBitMatrix) {
        if (sizeIfCopy <= sizeIfCouplesij) {
            ...
            (constraints16 ? mingraph_writeCopy16 : mingraph_writeCopy32)(mingraph + offset, dbm,
                                                                          dim);
    ...
    } else /* sizeIfCopy > sizeIfBitMatrix */
    {
        ...
        if (sizeIfCouplesij >= sizeIfBitMatrix) {
            ...
            (constraints16 ? mingraph_writeMinBitMatrix16 : mingraph_writeMinBitMatrix32)(
                mingraph + offset, dbm, dim, bitMatrix, cnt);
            return mingraph;
        }
        /* else we have
    ...
    (constraints16 ? mingraph_writeMinCouplesij16 : mingraph_writeMinCouplesij32)(
        mingraph + offset, dbm, dim, bitMatrix, cnt, bitCode);
    return mingraph;
}
...
static size_t mingraph_analyzeForMinDBM(const raw_t* dbm, cindex_t dim, uint32_t* bitMatrix)
{
    ...
    if (cnt != 0xffffffff) {
         ...
         */
        assert(sizeof(cindex_t) == sizeof(int32_t) &&
               "assuming cindex_t is alias of 4-byte structure");
                ...
                *end = i;
                ...
                if (i + 1 < dim) {
                    ...
                    do {
                         ...
                         */
                        assert(dbm_raw2bound(dbm[i * dim + j]) + dbm_raw2bound(dbm[j * dim + i]) !=
                                   0 ||
                                         ...
                                         const uint32_t* bitMatrix, uint32_t cnt)
{
    ...
    *where = dim | 0x00040000 | /* minimal graph */
     ...
     */
    memcpy(mingraph_jumpInt16(constraints, cnt), bitMatrix,
           bits2intsize(dim * dim) * sizeof(uint32_t));
