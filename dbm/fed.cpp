...
#warning "Bad subtraction: Skip non intersecting facettes."
    ...
    // Approximate test.
    static inline bool isInside(const raw_t* dbm1, const raw_t* dbm2, cindex_t dim, cindex_t i,
                                cindex_t j)
    {
        cindex_t k = 0;
        size_t kdim = 0;
    ...
    static dbmlist_t internSubtract(fdbm_t* fdbm1, const raw_t* dbm2, cindex_t dim,
                    ...
                    if (dbm2[i * dim + j] < dbm1[i * dim + j]) {
                        ...
                        if (negConstraint < dbm1[j * dim + i]) {
                            if (nb == 0 && nbConstraints == 0) {
                                dbm_tighten(isMutable ? dbm1 : fdbm1->dbmt().getCopy(), dim, j, i,
                                            negConstraint);
                                INC();
...
#warning "Subtraction: Result not disjoint."
    ...
    static int32_t worstValue(const raw_t* dbm1, const raw_t* dbm2, cindex_t dim, cindex_t i,
                              cindex_t j)
    {
        cindex_t k = 0;
        size_t kdim = 0;
                                    ...
                                    const uint32_t* bits, size_t bitsSize, size_t nbConstraints)
    {
        ...
        uint32_t k = static_cast<uint32_t>(nbConstraints);
...
#error "Invalid algorithm."
    ...
    {
        return (cij != dbm_LS_INFINITY && cji != dbm_LS_INFINITY &&
                ...
                for (fdbm_t** fj = head; fj != fi;) {
                    ...
                    for (cindex_t i = 1; i < dim; ++i) {
                        ...
                        for (cindex_t j = 0; j < i; ++j) {
                            ...
                            constraintsOK = constraintsOK | ((dbm1[ij] == dbm2[ij]) &  // | possible
                                                             (dbm1[ji] == dbm2[ji]));
                        }
                        nbOK += constraintsOK;
                    }
                        ...
                        *fj = (*fj)->removeAndNext();
                        ...
                        if (!safeMerge && level) {
                            ...
                            if (level == 1) {
                                ...
                                for (fdbm_t** fk = head; *fk != nullptr;
                                     fk = (*fk)->getNextMutable()) {
                                    if (fk != fi && fk != fj) {
        ...
        if (dim != arg.getDimension()) {
            ...
            for (const_iterator iter = begin(); i < thisSize; ++i, ++iter) {
                ...
                for (const_iterator jter = arg.begin(); j < argSize; ++j, ++jter) {
                    ...
                    if ((base_sub2super((relation_t)crossRel[i]) & crossRel[j] & base_SUPERSET) ==
                        0) {
                        relation_t rel =
                            dbm_relation(iter->const_dbm(), jter->const_dbm() /*argDBM[j]*/, dim);
            ...
            do {
                subset &= crossRel[i];
            } while (++i < thisSize && subset);
            i = 0;
            do {
                superset &= crossRel[i];
            } while (++i < argSize && superset);
    ...
    relation_t fed_t::exactRelation(const fed_t& arg) const
    {
        ...
        if (sameAs(arg)) {
            ...
            bool thisIncluded = isSubtractionEmpty(arg);
            bool argIncluded = arg.isSubtractionEmpty(*this);
    ...
    relation_t fed_t::exactRelation(const raw_t* arg, cindex_t dim) const
    {
        ...
        if (dim != getDimension()) {
            ...
            bool thisIncluded = isSubtractionEmpty(arg, dim);
            bool argIncluded = isSubtractionEmpty(arg, dim, *this);
    ...
    fed_t& fed_t::operator-=(const dbm_t& arg)
    {
        ...
        if (!(isEmpty() || arg.isEmpty())) {
            RECORD_STAT();
            RECORD_SUBSTAT(getDimension() == 1 ? "dim==1"
                                               : (*this <= arg ? "*this<=arg" : "non-trivial"));
            ...
            compute_convexi:
                ...
                if (removedj.size() > 0)  // otherwise convexi == dbmi
                {
                    ...
                    if (tooMuch.isEmpty()) {
                        ...
                        {
                            CERR(GREEN(BOLD) "R(" << (1 + removedj.size() - newFed.size())
                                                  << ")" NORMAL);
                        ...
                        if (removedj.add(dbmi)
                                .expensiveReduce()
                                .ifedPtr->const_head()
                                ->const_dbmt()
                                .sameAs(dbmi)) {
                            CERR(RED(THIN) "B(" << (newFed.size() - 1 - removedj.size())
                                                << ")" NORMAL);
    ...
    fed_t& fed_t::partitionReduce()
    {
        ...
        if (size() == 2) {
            ...
            for (fdbm_t** head = ifed()->atHead(); *head != nullptr;) {
                ...
                for (fi = *endi, endi = (*endi)->getNextMutable(); fi != nullptr;
                     fi = fi->getNext()) {
                    const raw_t* dbmi = fi->const_dbmt().const_dbm();
                    assert(*endi == nullptr);  // end of partition
                    // Loop on DBMs j of this federation
                    for (fdbm_t** fj = head; *fj != nullptr;) {
                        if (dbm_relaxedIntersection(tmp.dbm(), dbmi,
                                                    (*fj)->const_dbmt().const_dbm(), dim)) {
    ...
    bool fed_t::getMinDelay(const double* point, cindex_t dim, double* t, double* minVal,
                            bool* isStrict, const uint32_t* stopped) const
    {
        assert(isOK());
        assert(dim == getDimension());
        ...
        *t = HUGE_VAL;
        ...
        bool strict = false;
        double* boundPtr = minVal ? &bound : nullptr;
        bool* strictPtr = isStrict ? &strict : nullptr;
        for (const_iterator i = begin(); !i.null(); ++i) {
    ...
    void fed_t::resize(const uint32_t* bitSrc, const uint32_t* bitDst, size_t bitSize,
                       cindex_t* table)
    {
        assert(isOK());
    ...
    fed_t fed_t::subtract(const raw_t* arg1, const raw_t* arg2, cindex_t dim)
    {
        ...
        if (dim <= 1) {
            ...
            if (dbm_haveIntersection(arg1, arg2, dim)) {
                ...
                size_t nb =
                    dbm_cleanBitMatrix(arg2, dim, minDBM, dbm_analyzeForMinDBM(arg2, dim, minDBM));
                if (nb == 0) {
                    // arg2 is unconstrained => result = empty
                    return fed_t(dim);
                } else {
                    return fed_t(ifed_t::create(dim, internSubtract(fdbm_t::create(arg1, dim), arg2,
                                                                    dim, minDBM, minSize, nb)));
                }
            }
            // arg1 - arg2 = arg1
    ...
    fed_t fed_t::subtract(const dbm_t& arg1, const dbm_t& arg2)
    {
        ...
        if (arg1.isEmpty() || dim <= 1 || arg1.sameAs(arg2)) {
            ...
            return fed_t(ifed_t::create(dim, internSubtract(fdbm_t::create(arg1), arg2.const_dbm(),
                                                            dim, minDBM, minSize, nb)));
        } else {
            // arg1 - arg2 = arg1
            return fed_t(arg1);
            ...
            if (dbm_haveIntersection(arg1.const_dbm(), arg2, dim)) {
                ...
                if (nb == 0) {
                    ...
                    return fed_t(ifed_t::create(
                        dim, internSubtract(fdbm_t::create(arg1), arg2, dim, minDBM, minSize, nb)));
                }
            }
            // arg1 - arg2 = arg1
            return fed_t(arg1);
        ...
        if (dim <= 1) {
            ...
            do {
                ...
                if (dbm_haveIntersection(current->dbmt().const_dbm(), arg.const_dbm(), dim)) {
                    ...
                    dbmlist_t partial =
                        internSubtract(current, arg.const_dbm(), dim, minDBM, minSize, nb);
                    result.unionWith(partial);
                } else  // current - arg = current
                {
                    result.append(current);
