...
#define for_each_const(Z) for (const_iterator Z = begin(), Z_ = end(); Z != Z_; zone++)
    ...
    {
        return find_if(begin(), end(), bind(pdbm_satisfies, _1, ptr->dim, i, j, constraint)) !=
               end();
    }

    bool pfed_t::isUnbounded() const
    {
        return find_if(begin(), end(), bind(pdbm_isUnbounded, _1, ptr->dim)) != end();
    }
    ...
    {
        return find_if(begin(), end(), bind(pdbm_containsDouble, _1, ptr->dim, valuation())) !=
               end();
    }

    bool pfed_t::containsWeakly(const IntValuation& valuation) const
    {
        return find_if(begin(), end(), bind(pdbm_containsIntWeakly, _1, ptr->dim, valuation())) !=
               end();
    }
    ...
    relation_t pfed_t::relation(const pfed_t& b) const
    {
        ...
        return (relation_t)((p_a == a_s + size_a ? base_SUBSET : 0) |
    ...
    {
        for_each(beginMutable(), endMutable(), bind(pdbm_freeUp, _1, ptr->dim, i));
    }

    void pfed_t::freeDown(cindex_t i)
    {
        for_each(beginMutable(), endMutable(), bind(pdbm_freeDown, _1, ptr->dim, i));
    }
    ...
    void pfed_t::splitExtrapolate(const constraint_t* begin, const constraint_t* end,
                                  const int32_t* max)
    {
