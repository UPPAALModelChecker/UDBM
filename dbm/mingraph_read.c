...
static size_t mingraph_bitMatrixFromCopy32(uint32_t* bitMatrix, const int32_t* mingraph,
                                           bool isUnpacked, raw_t* buffer);
static size_t mingraph_bitMatrixFromCopy16(uint32_t* bitMatrix, const int32_t* mingraph,
                                           bool isUnpacked, raw_t* buffer);
static size_t mingraph_bitMatrixFromBitMatrix32(uint32_t* bitMatrix, const int32_t* mingraph,
                                                bool isUnpacked, raw_t* buffer);
static size_t mingraph_bitMatrixFromBitMatrix16(uint32_t* bitMatrix, const int32_t* mingraph,
                                                bool isUnpacked, raw_t* buffer);
static size_t mingraph_bitMatrixFromCouplesij32(uint32_t* bitMatrix, const int32_t* mingraph,
                                                bool isUnpacked, raw_t* buffer);
static size_t mingraph_bitMatrixFromCouplesij16(uint32_t* bitMatrix, const int32_t* mingraph,
                                                bool isUnpacked, raw_t* buffer);
static size_t mingraph_bitMatrixError(uint32_t* bitMatrix, const int32_t* mingraph, bool isUnpacked,
                                      raw_t* buffer);
...
#ifdef EXPERIMENTAL
/* Experimental close */
static void mingraph_close(uint32_t* indices, size_t n, raw_t* dbm, cindex_t dim);
#endif
...
{
    static const hasZero_f hasZero[8] = {mingraph_copy32HasZero, mingraph_copy16HasZero,
                                         mingraph_errorHasZero,  mingraph_errorHasZero,
                                         mingraph_c32HasZero,    mingraph_c16HasZero,
                                         mingraph_c32HasZero,    mingraph_c16HasZero};
...
cindex_t dbm_readFromMinDBM(raw_t* dbm, const int32_t* minDBM)
{
     ...
     */
    static const readDBM_f readFromMinDBM[8] = {mingraph_readFromCopy32,
    ...
    /* dim > 1
     */
    return readFromMinDBM[mingraph_getTypeIndexFromPtr(minDBM)](dbm, minDBM);
}
 ...
 * 2) call the proper decoding function
 */
size_t dbm_getBitMatrixFromMinDBM(uint32_t* bitMatrix, const int32_t* minDBM, bool isUnpacked,
                                  raw_t* buffer)
{
    /* see mingraph_getTypeIndex comments
     */
    static const bitMatrixDBM_f bitMatrixFromMinDBM[8] = {
        mingraph_bitMatrixFromCopy32,      mingraph_bitMatrixFromCopy16,
        mingraph_bitMatrixError,           mingraph_bitMatrixError,
        mingraph_bitMatrixFromBitMatrix32, mingraph_bitMatrixFromBitMatrix16,
        mingraph_bitMatrixFromCouplesij32, mingraph_bitMatrixFromCouplesij16};
    ...
    /* dim > 1
     */
    return bitMatrixFromMinDBM[mingraph_getTypeIndexFromPtr(minDBM)](bitMatrix, minDBM, isUnpacked,
                                                                     buffer);
}
...
{
    fprintf(stderr,
            RED(BOLD) PACKAGE_STRING " fatal error: invalid encoded DBM to test" NORMAL "\n");
    exit(2);
    return false;
}
...
{
    fprintf(stderr,
            RED(BOLD) PACKAGE_STRING " fatal error: invalid encoded DBM to read" NORMAL "\n");
    exit(2);
...
{
    cindex_t dim =
        isUnpacked ? mingraph_readDimFromPtr(mingraph) : mingraph_readFromCopy32(buffer, mingraph);
...
{
    cindex_t dim =
        isUnpacked ? mingraph_readDimFromPtr(mingraph) : mingraph_readFromCopy16(buffer, mingraph);
    assertx(!isUnpacked || dbm_isEqualToMinDBM(buffer, dim, mingraph));
    return dbm_analyzeForMinDBM(buffer, dim, bitMatrix);
}
 ...
 */
static size_t mingraph_addMissingConstraints(uint32_t* matrix, const int32_t* mingraph, raw_t* dbm,
                                             cindex_t dim, size_t nbConstraints)
{
    if (!CLOCKS_POSITIVE) {
        // Analysis and correction unnecessary.
    ...
    for (q = first + 1; q < end; q++)
        if (dbm[*q] == dbm_LE_ZERO && !base_readOneBit(matrix, *q)) {
            for (r = first + 1; r < end; r++)
                if (r != q) {
                    raw_t bik = dbm[*r];
 ...
 * Complexity: O(dim^2)
 */
static size_t mingraph_bitMatrixFromBitMatrix32(uint32_t* matrix, const int32_t* mingraph,
                                                bool isUnpacked, raw_t* buffer)
{
    cindex_t dim = mingraph_readDimFromPtr(mingraph);
    size_t nbConstraints = mingraph_getNbConstraints(mingraph);
    ...
    /* copy bit matrix */
    memcpy(matrix, minMatrix, bits2intsize(dim * dim) * sizeof(int32_t));
...
/* Similar to previous */
static size_t mingraph_bitMatrixFromBitMatrix16(uint32_t* matrix, const int32_t* mingraph,
                                                bool isUnpacked, raw_t* buffer)
{
    cindex_t dim = mingraph_readDimFromPtr(mingraph);
    size_t nbConstraints = mingraph_getNbConstraints(mingraph);
...
static size_t mingraph_bitMatrixFromCouplesij32(uint32_t* matrix, const int32_t* mingraph,
                                                bool isUnpacked, raw_t* dbm)
{
    ...
    assert(dbm && (dim > 2 || nbConstraints == 0));
    // cast because C is lacking templating.
    memset(matrix, 0, bits2intsize(dim * dim) * sizeof(uint32_t));
...
/* Similar to previous */
static size_t mingraph_bitMatrixFromCouplesij16(uint32_t* matrix, const int32_t* mingraph,
                                                bool isUnpacked, raw_t* dbm)
{
    uint32_t info = mingraph_getInfo(mingraph);
    cindex_t dim = mingraph_readDim(info);
    ...
    memset(matrix, 0, sizeof(uint32_t) * bits2intsize(dim * dim));
    if (!isUnpacked) {
        dbm_init(dbm, dim);
    }
...
#ifdef EXPERIMENTAL
/* Experimental close */
static void mingraph_close(uint32_t* indices1, size_t n, raw_t* dbm, cindex_t dim)
{
