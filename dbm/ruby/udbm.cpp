...
{
    check_dim(f1->getDimension(), f2->getDimension());
}
...
{
    return RELATION2VALUE(check_relation(arg));
}
...
static VALUE cRelation_and(VALUE self, VALUE arg)
{
    return RELATION2VALUE(value2relation(self) & check_relation(arg));
}
            ...
            for (cindex_t i = 0; i < dim; ++i) {
                for (cindex_t j = 0; j < dim; ++j) {
                    ...
                    if (c == dbm_LS_INFINITY) {
                        ...
                        snprintf(number, sizeof(number), "\t%s%d",
                                 dbm_rawIsStrict(c) ? "<" : "<=", dbm_raw2bound(c));
                        str += number;
                    }
                }
                str += " \\\n";
    ...
    for (fed_t::const_iterator iter = fed->begin(); !iter.null(); ++iter) {
        ...
        if (dbm) {
            ...
            for (size_t i = 0; i < matSize; ++i) {
                ...
                rb_ary_store(
                    arr, i,
                    rb_funcall(cConstraint, NEW_ID, 2, INT2FIX(bound), isStrict ? Qtrue : Qfalse));
            }
...
{
    CONST_DO(do_extrapolateMaxBounds(&result, arg));
}
static VALUE cFed_self_extrapolateMaxBounds(VALUE self, VALUE arg)
{
    SELF_DO(do_extrapolateMaxBounds(fed, arg));
}
...
static void array2bits(VALUE arg, uint32_t* bits, cindex_t dim)
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (clock == 0 || clock >= (int)dim) {
            rb_raise(rb_eArgError, "Invalid clock argument (%d). Clock must be > 0 and < %d.",
                     clock, dim);
        }
...
static VALUE cFed_const_getDrawing(VALUE self, VALUE vborder, VALUE vwidth, VALUE vheight, VALUE vx,
                                   VALUE vy)
{
    FED(self, fed);
    cindex_t dim = fed->getDimension();
                    ...
                    if (base_readOneBit(minGraph, i * dim + j)) {
                        ...
                        if (i == 0)  // don't print ref clock
                        {
                            ...
                            if (dbm_addFiniteRaw(m[j], m[j * dim]) == dbm_LE_ZERO) {
                                ...
                                // xi == b
                                snprintf(buffer, sizeof(buffer), "%s==%d", names[j - 1],
                                         -dbm_raw2bound(m[j]));
                            } else {
                                // -b <= xi
                            ...
                            if (dbm_addFiniteRaw(m[i * dim + j], m[j * dim + i]) == dbm_LE_ZERO) {
                                ...
                                if (j > 0 && m[i * dim + j] == dbm_LE_ZERO) {
                                    snprintf(buffer, sizeof(buffer), "%s==%s", names[i - 1],
                                             names[j - 1]);
                                } else if (j == 0) {
                                    snprintf(buffer, sizeof(buffer), "%s==%d", names[i - 1],
                                             dbm_raw2bound(m[i * dim]));
                                } else {
                                    snprintf(buffer, sizeof(buffer), "%s-%s==%d", names[i - 1],
                                             names[j - 1], dbm_raw2bound(m[i * dim + j]));
                                }
                            } else {
                                // xi < xj, xi <= xj
                                if (j > 0 && dbm_raw2bound(m[i * dim + j]) == 0) {
                                    snprintf(
                                        buffer, sizeof(buffer), "%s%s%s", names[i - 1],
                                        dbm_rawIsStrict(m[i * dim + j]) ? "<" : "<=", names[j - 1]);
                                } else if (j == 0) {
                                    snprintf(buffer, sizeof(buffer), "%s%s%d", names[i - 1],
                                             dbm_rawIsStrict(m[i * dim + j]) ? "<" : "<=",
                                             dbm_raw2bound(m[i * dim + j]));
                                } else {
                                    snprintf(buffer, sizeof(buffer), "%s-%s%s%d", names[i - 1],
...
{
    CONST_DO(do_changeClocks(&result, arg));
}
...
#define SINGLETON(CLASS, NAME, FUNC, ARGC) \
    rb_define_singleton_method(CLASS, NAME, VALUEFUNC(FUNC), ARGC)
...
RelationCache::entry_t* RelationCache::get_entry(fed_t& f1, fed_t& f2)
{
    ...
    uint32_t hashValue =
        hash_computeU32((uint32_t*)feds.fed2, feds.size2,
    ...
    for (bucket = *atBucket; bucket != NULL; bucket = bucket->getNext()) {
        if (bucket->info == hashValue && feds.size1 == bucket->feds.size1 &&
            feds.size2 == bucket->feds.size2 &&
