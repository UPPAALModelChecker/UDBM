...
static relation_t mingraph_relationWithCopy32(const raw_t* dbm, cindex_t dim,
                                              const int32_t* mingraph, raw_t* unpackBuffer,
                                              bool* isExact);
static relation_t mingraph_relationWithCopy16(const raw_t* dbm, cindex_t dim,
                                              const int32_t* mingraph, raw_t* unpackBuffer,
                                              bool* isExact);
static relation_t mingraph_relationWithMinBitMatrix32(const raw_t* dbm, cindex_t dim,
                                                      const int32_t* mingraph, raw_t* unpackBuffer,
                                                      bool* isExact);
static relation_t mingraph_relationWithMinBitMatrix16(const raw_t* dbm, cindex_t dim,
                                                      const int32_t* mingraph, raw_t* unpackBuffer,
                                                      bool* isExact);
static relation_t mingraph_relationWithMinCouplesij32(const raw_t* dbm, cindex_t dim,
                                                      const int32_t* mingraph, raw_t* unpackBuffer,
                                                      bool* isExact);
static relation_t mingraph_relationWithMinCouplesij16(const raw_t* dbm, cindex_t dim,
                                                      const int32_t* mingraph, raw_t* unpackBuffer,
                                                      bool* isExact);
static relation_t mingraph_relationError(const raw_t* dbm, cindex_t dim, const int32_t* mingraph,
 ...
 * not exact relations.
 */
static const relation_f relationWithMinDBM[8] = {mingraph_relationWithCopy32,
 ...
 */
relation_t dbm_relationWithMinDBM(const raw_t* dbm, cindex_t dim, const int32_t* minDBM,
                                  raw_t* unpackBuffer)
{
    uint32_t info;
     ...
     */
    if (*minDBM == 1) {
        return (dim == 1 && *dbm == dbm_LE_ZERO) ? (unpackBuffer ? base_EQUAL : base_SUBSET)
                                                 : base_DIFFERENT;
    }
     ...
     */
    rel = relationWithMinDBM[mingraph_getTypeIndex(info)](dbm, dim, minDBM, unpackBuffer, &isExact);
 ...
 */
relation_t dbm_approxRelationWithMinDBM(const raw_t* dbm, cindex_t dim, const int32_t* minDBM,
                                        raw_t* unpackBuffer)
{
    uint32_t info;
    bool isExact = false; /* by default */
 ...
 */
static relation_t mingraph_relationWithMinBitMatrix32(const raw_t* dbm, cindex_t dim,
                                                      const int32_t* mingraph, raw_t* needsExact,
                                                      bool* isExact)
{
                                                      ...
                                                      bool* isExact)
{
    ...
    if (nbConstraints) /* could be = 0 */
    {
         ...
         * or 1 << (code + 2)
         */
        int bitSize = 1 << (mingraph_typeOfIJ(info) + 2);
        int bitMask = (1 << bitSize) - 1; /* standard */
        const raw_t* constraints = mingraph_getCodedData(mingraph);
                                         ...
                                         raw_t* unpackBuffer, bool* isExact)
{
    fprintf(stderr, RED(BOLD) PACKAGE_STRING
            " fatal error: invalid encoded DBM to compute relation with" NORMAL "\n");
    exit(2);
