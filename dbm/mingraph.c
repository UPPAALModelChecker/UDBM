 ...
 */
void dbm_bitMatrix2indices(const uint32_t* bits, size_t nbConstraints, uint32_t* index,
                           cindex_t dim)
{
    uint32_t i, j, k;
...
/* Decode information.
 */
cindex_t dbm_getDimOfMinDBM(const int32_t* minDBM) { return mingraph_readDimFromPtr(minDBM); }
...
size_t dbm_getSizeOfMinDBM(const int32_t* minDBM)
{
    ...
    if (mingraph_isMinimal(info)) {
        ...
        uint32_t headerAndConstraints =
            (uint32_t)1 + ((info & 0x00200000) >> 21) + ((nbConstraints + coded16) >> coded16);
        ...
        if (mingraph_isCodedIJ(info)) {
            ...
            return headerAndConstraints + bits2intsize(nbConstraints << (bitCode + 3));
        } else {
            /* add the bit matrix
             */
            cindex_t dim = mingraph_readDim(info);
            return headerAndConstraints + bits2intsize(dim * dim);
        }
    } else /* simply copy */
    {
        cindex_t dim = mingraph_readDim(info);
...
void dbm_convexUnionWithMinDBM(raw_t* dbm, cindex_t dim, const int32_t* minDBM, raw_t* unpackBuffer)
{
    ...
    if (dim > 1) {
        ...
        if (mingraph_isMinimal(info)) {
            ...
            (mingraph_isCoded16(info) ? mingraph_convexUnion16 : mingraph_convexUnion32)(
                dbm, minDBM, dim);
        }
    }
}
...
representationOfMinDBM_t dbm_getRepresentationType(const int32_t* minDBM)
{
    ...
    return (*minDBM == 1) ? dbm_MINDBM_TRIVIAL : codeTypes[mingraph_getTypeIndexFromPtr(minDBM)];
}
