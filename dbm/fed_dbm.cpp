...
namespace dbm
{
    ...
    std::string dbm_t::toString(const ClockAccessor& access, bool full) const
    {
        ...
        size_t n =
            full ? dbm_fillBitMatrix(dbm, dim, mingraph)
    ...
    bool dbm_t::canDelay() const
    {
        ...
        cindex_t dim = pdim();
        for (cindex_t i = 1; i < dim; ++i) {
            if (dbm_rawIsWeak(DBM(i, 0)) && dbm_rawIsWeak(DBM(0, i)) &&
                DBM(i, 0) == dbm_weakNegRaw(DBM(0, i))) {
                return false;
            }
        }
        return true;
    ...
    bool dbm_t::getMinDelay(const double* point, cindex_t dim, double* t, double* minVal,
                            bool* minStrict, const uint32_t* stopped) const
    {
                ...
                if (di < *t) {
                    ...
                    if (dbm_isRealPointIncluded(pt, dbm, dim)) {
                        ...
                        do {
                            // And validate di.
                            for (cindex_t i = 1; i < dim; ++i) {
                                pt[i] = stopped && base_readOneBit(stopped, i) ? point[i]
                                                                               : point[i] + di;
                            }
                            if (dbm_isRealPointIncluded(pt, dbm, dim)) {
                                *t = di;
    ...
    bool dbm_t::getMaxDelay(const double* point, cindex_t dim, double* t, double* minVal,
                            bool* minStrict, const uint32_t* stopped) const
    {
        assert(dim == getDimension() && point && t);
    ...
    void dbm_t::resize(const uint32_t* bitSrc, const uint32_t* bitDst, size_t bitSize,
                       cindex_t* table)
    {
        ...
        if (!base_areBitsEqual(bitSrc, bitDst, bitSize))  // pre-condition
        {
            ...
            if (isEmpty()) {
                ...
                idbm_t* old = idbmt();
                dbm_shrinkExpand(old->const_dbm(), setNew(newDim), old->getDimension(), bitSrc,
                                 bitDst, bitSize, table);
