 ...
 */
static int32_t costAtOtherOffset(const raw_t* dbm1, const int32_t* rates1, uint32_t cost1,
 ...
 */
static int32_t infOfDiff(const raw_t* dbm, uint32_t dim, int32_t cost1, const int32_t* rate1,
                         int32_t cost2, const int32_t* rate2)
{
...
relation_t pdbm_relation(const PDBM pdbm1, const PDBM pdbm2, cindex_t dim)
{
    ...
    case base_SUPERSET:
        ...
        if (cost1 <= cost2 && (leq(rates1, rates1 + dim, rates2) ||
    ...
    case base_SUBSET:
        ...
        if (cost2 <= cost1 && (leq(rates2, rates2 + dim, rates1) ||
...
relation_t pdbm_relationWithMinDBM(const PDBM pdbm1, cindex_t dim, const mingraph_t pdbm2,
                                   raw_t* dbm2)
{
...
int32_t pdbm_getInfimumValuation(const PDBM pdbm, cindex_t dim, int32_t* valuation,
                                 const bool* free)
{
...
int32_t* pdbm_writeToMinDBMWithOffset(const PDBM pdbm, cindex_t dim, bool minimizeGraph,
                                      bool tryConstraints16, allocator_t allocator, uint32_t offset)
{
    assert(pdbm && dim);

    int32_t* graph = dbm_writeToMinDBMWithOffset(pdbm_matrix(pdbm), dim, minimizeGraph,
                                                 tryConstraints16, allocator, offset + dim + 2);
