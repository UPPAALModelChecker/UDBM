...
static void test_diagonalExtrapolateMaxBounds(raw_t* dbm, cindex_t dim, const int32_t* max)
{
    ...
    for (i = 1; i < dim; ++i) {
        for (j = 0; j < dim; ++j) {
            if (i != j &&
                (dbm_raw2bound(DBM(0, i)) < -max[i] || dbm_raw2bound(DBM(0, j)) < -max[j] ||
...
static void test_diagonalExtrapolateLUBounds(raw_t* dbm, cindex_t dim, const int32_t* lower,
                                             const int32_t* upper)
{
    cindex_t i, j;
    ...
    for (i = 1; i < dim; ++i) {
        for (j = 0; j < dim; ++j) {
            if (i != j &&
                (dbm_raw2bound(DBM(i, j)) > lower[i] || dbm_raw2bound(DBM(0, i)) < -lower[i] ||
