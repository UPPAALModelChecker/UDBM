...
#define ASSERT_NOT_EMPTY(DBM, DIM) \
    ASSERT(dim == 0 || !dbm_isEmpty(DBM, DIM), dbm_print(stderr, DBM, DIM))
...
bool dbm_generate(raw_t* dbm, cindex_t dim, raw_t range)
{
    ...
    for (i = 1; i < dim; ++i) {
        for (j = 0; j < i; ++j) {
            ...
            int32_t maxTighten =
                nbTighten > threshold ? 5 * (rangeij - 1) / 12 : 7 * (rangeij - 1) / 12;
...
void dbm_generateArgDBM(raw_t* arg, const raw_t* dbm, cindex_t dim)
{
    ...
    case 0: /* no intersection at all */
    {
        ...
        cij.i = i;
        cij.j = j;
        cij.value = DBM(j, i) == dbm_LS_INFINITY ? RAND() % maxRange + 1
                                                 : /* can't get no intersection */
                        dbm_negRaw(DBM(j, i));
...
bool dbm_generateRealPoint(double* pt, const raw_t* dbm, cindex_t dim)
{
    ...
    for (i = 1; i < dim;) {
        ...
        for (j = 0; j < i; ++j) {
            ...
            double dbmji =
                DBM(j, i) < dbm_LS_INFINITY ? dbm_raw2bound(DBM(j, i)) : (dbm_LS_INFINITY >> 8);
            double dbmij =
                DBM(i, j) < dbm_LS_INFINITY ? dbm_raw2bound(DBM(i, j)) : (dbm_LS_INFINITY >> 8);
